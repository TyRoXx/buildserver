...
BOOST_AUTO_TEST_CASE(graph_typed_transformation)
{
	...
	graph::typed_transformation const tf_id_absolute_path{graph::atomic_type::absolute_path,
	                                                      graph::atomic_type::absolute_path, &identity};
	graph::typed_transformation const tf_id_blob{graph::atomic_type::blob, graph::atomic_type::blob, &identity};
	graph::typed_transformation const tf_uint32_to_le{graph::atomic_type::uint32, graph::atomic_type::blob,
	                                                  &uint32_to_le};
	graph::typed_transformation const tf_uint32_add{
	    Si::to_shared(make_pair_type(graph::atomic_type::uint32, graph::atomic_type::uint32)),
	    graph::atomic_type::uint32, &uint32_add};
	graph::value const result = tf_id_blob.transform(tf_uint32_to_le.transform(
	    tf_uint32_add.transform(Si::to_shared(make_pair(std::uint32_t(40), std::uint32_t(500))))));
	std::vector<char> const expected{28, 2, 0, 0};
	auto const *const blob_result = Si::try_get_ptr<graph::blob>(result);
	BOOST_REQUIRE(blob_result);
	BOOST_CHECK_EQUAL_COLLECTIONS(expected.begin(), expected.end(), blob_result->content.begin(),
	                              blob_result->content.end());
}
